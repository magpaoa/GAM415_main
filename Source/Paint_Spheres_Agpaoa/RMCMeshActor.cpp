...
ARMCMeshActor::ARMCMeshActor()
	: Material(nullptr)
{

}

void ARMCMeshActor::OnConstruction(const FTransform& Transform)
{
	...
	if (StaticProvider)
	{
		...
		StaticProvider->CreateSectionFromComponents(0, 0, 0,
			Vertices,
			Triangles,
			Normals,
			TextureCoordinates,
...
void ARMCMeshActor::GenerateBoxMesh()
{
	TArray<FVector> Vertices;
	TArray<FVector> Normals;
	TArray<FRuntimeMeshTangent> Tangents;
...
void ARMCMeshActor::CreateBoxMesh(FVector BoxRadius,
	TArray<FVector>& Vertices,
	TArray<int32>& Triangles,
	TArray<FVector>& Normals,
